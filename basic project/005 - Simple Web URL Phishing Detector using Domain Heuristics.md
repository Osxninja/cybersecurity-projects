...
Phishing attacks remain one of the most common and effective vectors for compromising user credentials and distributing malware. Attackers frequently use deceptive tactics, such as typosquatting (e.g., using 'paypa1.com' instead of 'paypal.com') or excessively long URLs, to trick non-technical users into visiting malicious websites. Identifying these fraudulent links automatically is a critical first line of defense in modern cybersecurity.

This project tackles the issue by introducing a Python-based URL analysis tool. By evaluating domain heuristics—like the presence of raw IP addresses, excessive hyphens, unusual string lengths, and suspicious keywords—the script calculates a risk score to determine the likelihood of a URL being malicious. It serves as an excellent practical introduction to threat intelligence and programmatic risk assessment.
...
1. A clear understanding of how domain heuristics and lexical features indicate malicious intent.
2. A functional, lightweight Python script capable of parsing and scoring URLs in a local environment.
3. The ability to identify common evasion techniques used by attackers in phishing campaigns.
